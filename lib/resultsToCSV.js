var resultsToCSV = (function () {
    ...
    var COMPLETED = {
        ...
        "experiments.wack_a_mole.code": {$exists: true, $nin: [""]}
    };
    ...
    var COLUMN_GENERATORS = {
        ...
        'block__score': function (worker, block_index) {
            var rounds = worker.experiments.wack_a_mole.data.blocks[block_index];
    ...
    function generateRow() {
        ...
        COLUMN_HEADERS.forEach(function (header) {
            row.push(COLUMN_GENERATORS[header].apply(null, args));
        });
        return row.join();
    }
    ...
    function generateSpreadsheet() {
        ...
        workers.forEach(function (worker) {
            worker.experiments.wack_a_mole.data.blocks.map(function (block, block_index) {
    ...
    function stringSpreadsheet() {
        return generateSpreadsheet().join('\r\n');
    }
