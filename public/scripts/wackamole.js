...
var WackAMoleApp = React.createClass({displayName: "WackAMoleApp",
  render: function () {
    ...
    if (this.state.round.number >= 0) {
      field = (React.createElement(Field, {dimensions: this.props.settings.dimensions, row: this.state.round.mole_row, patch: this.state.round.mole_col, hit: this.moleHit, miss: this.moleMiss}));
    };

    return (
      React.createElement("div", null, 
        React.createElement("h2", null, "Wack-A-Mole"), 
        React.createElement("p", null, "You have ", this.props.settings.wait_times.length, " chances to wack that mole!"), 
        React.createElement("ul", null, 
          React.createElement("li", null, HIT, " points for every mole wacked!"), 
          React.createElement("li", null, MISS, " points for each wack that misses!"), 
          React.createElement("li", null, DOWN, " points for each mole that hides before you wack them!")
        ), 
        React.createElement("p", null, React.createElement("b", null, "Score: "), this.state.round.score), 
        React.createElement("input", {type: "btn", className: "btn btn-primary", value: "Start!", onClick: this.startRound, disabled: this.state.round.number >= 0}), 
      field
      )
    );
  },
...
React.render(
  React.createElement(CrowdExperiment, {experiment_name: "wack_a_mole", experiment_app: WackAMoleApp}),
